...
window.addEventListener("load", async () => {
  ...
  const subCateroriesResponse = await subCategoriesResult.json();
  const subCategories = subCateroriesResponse.data.categories;
  ...
  searchInput.addEventListener("keyup", (e) => {
    if (searchInput.value.trim()) {
      ...
      resultContaienrModal.classList.add("active");
      removeIcon.classList.add("active");
      if (filteredCategories.length > 0) {
        resultContaienrModal.textContent = "";
        filteredCategories.map((category) => {
          resultContaienrModal.insertAdjacentHTML(
            "beforeend",
            `
     ...
     `,
      ...
      searchInput.value = "";
      resultContaienrModal.textContent = "";
      resultContaienrModal.classList.remove("active");
